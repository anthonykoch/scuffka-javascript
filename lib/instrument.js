...
export default ({ types: t, ast, traverse }) => {
  ...
  const addInsertionPoint = (node, origin, isExpression=false) => {
    id += 1;
  ...
  const track = (node, forceSequence=false, origin=node) => {
    const insertionId = addInsertionPoint(node, node, true);
