...
export const wrap = (code, args, {
  id = '',
  closure = false
}) => {
  const parameters = args.map(str => str.replace(/\s/g, '')).join(', ');
  const header = `${closure ? 'return ' : ';'}(function ${id}(${parameters}) {`;
  const footer = `});`;
  return {
    header,
    footer,
...
export const run = (input, options = {}) => {
  ...
  try {
    exec(input, {
      functionId,
      filename: __filename,
      parameters: ['exports', 'require', 'module', '__filename', '__dirname', VAR_INSPECT, VAR_NOTIFY_BLOCK],
